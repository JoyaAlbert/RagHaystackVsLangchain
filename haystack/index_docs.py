...
# ---- Seguridad y rendimiento: limitar hilos y forzar CPU ----
...
def create_client(persist_dir: str):
    """Crea un cliente ChromaDB con persistencia habilitada."""
...
def index_files(
    ...
    """Indexa los documentos en ChromaDB, evitando duplicados."""
        ...
        for mlist in existing.get('metadatas', []):
            for m in mlist:
                if isinstance(m, dict) and 'source' in m:
                    ...
                    # keep the first seen hash (they should be identical for same source)
    ...
    for path in files:
        ...
        try:
            ...
            if not text or not text.strip():
                logging.warning(f'Archivo vacío o ilegible: {abs_path}, se omite')
                continue
            ...
            # Procesar por lotes (batch) para evitar sobrecarga de memoria
            for i in range(0, len(chunks), batch_size):
                batch_chunks = chunks[i:i + batch_size]
                # Use a short id derived from path and file-hash to reduce collisions
            ...
            # Guardar progreso tras cada archivo
            try:
                client.persist()
            except Exception:
                pass
...
def main():
    load_dotenv()
    parser = argparse.ArgumentParser(description='Indexador de documentos en ChromaDB')
